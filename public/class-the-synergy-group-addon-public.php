...
class The_Synergy_Group_Addon_Public
{
	...
	public function enqueue_styles()
	{
		...
		if (is_account_page()) {
			wp_enqueue_style($this->plugin_name . '-my-account', plugin_dir_url(__FILE__) . 'css/myaccount.css', array(), '1.0.5', 'all');
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script('chartjs', 'https://cdn.jsdelivr.net/npm/chart.js', array('jquery'), '1.0.0', true);
		wp_enqueue_script('synergy-chart-script', plugin_dir_url(__FILE__) . 'js/chart-script.js', array('jquery', 'chartjs'), '1.0.0', true);

		$chart_data = synergy_get_chart_data();

		wp_localize_script('synergy-chart-script', 'chartData', $chart_data);
	}
}
